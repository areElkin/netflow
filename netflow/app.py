...
class App:
    ...
    """
    def __init__(self, G, R, R_thresh=0.9, selected_graph='combo', node_type=int, **pos_kwargs):
        ...
        self.app.layout = html.Div(
            [
                ...
                dbc.Row([dbc.Col(dcc.RadioItems(options=[{'label': 'Interaction network', 'value': 'interaction'},
                         ...
                         dbc.Col(html.Div([html.Div("Node positions"),
                                           html.Button('Fix positions', id='btn_fix_pos', n_clicks=0, disabled=False),
                                           html.Button('Reset positions', id='btn_reset_pos', n_clicks=0, disabled=True)]),
                                          width=1),
                         dbc.Col(html.P("Info", id="cyto_info"), width=6)], className="nine columns"),
        ...
        self.app.callback(Output('cytoscape_visualization', 'elements'),
                          Input('radio_graph_selection', 'value'))(self.update_elements)
    ...
    # @self.app.callback(Output('cytoscape_visualization', 'elements'),
    #                    Input('radio_graph_selection', 'value'))
    def update_elements(self, selected_graph):
        self.selected_graph = selected_graph
